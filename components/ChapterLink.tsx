...
const ChapterLink = ({ levelId, title }: any) => {
  return (
    <Suspense fallback={<Skeleton className='h-12 w-12 rounded-full  ' />}>
      <Link
        key={title.id}
        href={
          '/studentDashboard/levels/' + levelId + '/chapters/' + title.number
        }
        passHref
      >
        <div className='group  p-4  flex  items-center justify-start hover:bg-secondary/90 bg-secondary/50 rounded-3xl  w-full '>
          ...
          <div className='p-4 flex flex-col items-start justify-between'>
            <small className='text-default-500'> Niveau {title.level} </small>
            <h1 className=' font-black text-2xl '> {title.title}</h1>
          </div>
        </div>
      </Link>
    </Suspense>
